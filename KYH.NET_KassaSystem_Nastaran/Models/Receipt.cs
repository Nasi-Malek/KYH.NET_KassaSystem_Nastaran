...
namespace KYH.NET_KassaSystem_Nastaran.Models
{
    ...
    private static string receiptCounterFilePath = "ReceiptCounter.txt"; // Sökväg för filen som lagrar kvittonumret
        ...
        public void AddItem(Product product, int quantity)
        {
            ...
            if (quantity <= 0) throw new ArgumentException("Antal måste vara positivt.", nameof(quantity));

            try
            {
                Items.Add((product, quantity));
                //Console.WriteLine("Produkt tillagd i kvittot.");
            }
            catch (Exception ex)
            {
        ...
        public void PrintAndSaveReceipt()
        {
            ...
            using (StreamWriter writer = new StreamWriter(filePath, true)) // Öppna filen i tilläggsläge
            {
                ...
                // Lägg till varje artikel i kvittot
                foreach (var item in Items)
                {
                    decimal itemTotal = item.product.GetEffectivePrice(Date) * item.quantity;
                    decimal effectivePrice = item.product.GetEffectivePrice(Date);
                    writer.WriteLine($"{item.product.Name} \t\t{item.quantity} * {item.product.GetEffectivePrice(Date)} kr = {itemTotal:0.00} kr "); // Skriv ut produktnamn, kvantitet och totalpris
